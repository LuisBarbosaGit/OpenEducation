import { CodeIcon, LoaderCircleIcon, MonitorCogIcon, SendHorizonalIcon } from "lucide-react";
import { useContext, useEffect, useRef, useState } from "react"
import type { responseModel } from "../../models/response-model";
import { responseSchema } from "../../schema/response-schema";
import z from "zod";
import { DropDownHome } from "@/components/DropDownHome/DropDownHome";
import { FlagMenuContext } from "@/components/DropDownHome/DropDownHomeContext";
import {MainServices} from "@/services/MainService";

export const options : string[] = ['Gerar Código', 'Cypress']

export const icons : React.ReactNode[] = [<CodeIcon className="group-focus:text-stone-300"/>, <MonitorCogIcon className="group-focus:text-stone-300:" />]
...
export const Home = () => {
  ...
  const {flagItem} = useContext(FlagMenuContext)
        ...
        <form action="submit" id="formMain" onSubmit={(e) => {handleFormSubmit(e)}} className="flex flex-col">
          <div className="max-h-60 overflow-y-auto">
            <label about="query" className="flex border-3 border-stone-400  bg-stone-600 rounded-3xl items-center max-h-60 min-h-6 overflow-y-auto  ">
              <div className="flex items-center justify-center h-full p-2 group ">
                <DropDownHome options={options} icon={icons}/>
              </div>
              <textarea
                className="focus:outline-none focus:text-white transition-all focus:placeholder:text-stone-800 placeholder:text-stone-300 duration-100 ease-in bg-stone-600 text-center font-roboto w-[42rem] text-stone-300  overflow-hidden resize-none  leading-tight text-lg py-3"
